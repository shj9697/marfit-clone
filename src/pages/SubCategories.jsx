...
function SubCategories() {
	...
	const handleSortBy = (value = 'relevance') => {
		setSortBy(value);
	}

	const handleCategoryBy = (value = 'category') => {
		setCategory(value);
	}

	const handleSubCategoryBy = (value = 'subCategory') => {
	...
	const filterIcon = "data:image/png;base64,"
	return (
		<div className="w-full bg-white">
			<Breadcrumb
				paths={[
					{
						title: parentId,
						link: `/categories/${parentId}`,
					},
					{
						title: subId,
						link: `/categories/${parentId}/${subId}`,
					},
				]}
			/>
					...
					<div className="flex flex-col w-70">
						<div className="flex">
							<div className="flex w-full">
								<img src={filterIcon} alt="" className="h-5 w-5 my-1" />
								<p className="text-xl">Filters</p>
							</div>
							<button className="p-1 border border-gray-200 w-[30%] rounded cursor-pointer">Reset</button>
						</div>
						<div className="w-70 accent-orange-600 flex flex-col my-2 border border-gray-200 rounded-md p-4">
							<h1>SORT BY</h1>
							<div className="flex">
								<input
									type="checkbox"
									checked={sortBy === 'relevance'}
									name="relevance" id="sort-by-relevance"
									className="my-2 mx-2 h-4 w-4"
									onClick={() => handleSortBy('relevance')}
								/>
								<label>RELEVANCE</label>
							</div>
							<div className="flex">
								<input type="checkbox" checked={sortBy === 'price-low-to-high'} name="Price Low to High" id="sort-by-price-low-to-high" className="my-2 mx-2 h-4 w-4" onClick={() => handleSortBy('price-low-to-high')} />PRICE-LOW TO HIGH
							</div><div className="flex">
								<input type="checkbox" checked={sortBy === 'price-high-to-low'}
									name="price high to low" id="price-high-to-low"
									className="my-2 mx-2 h-4 w-4"
									onClick={() => handleSortBy('price-high-to-low')} />PRICE-HIGH TO LOW
							</div>
						</div>
						<div className="border border-gray-200 rounded-md  accent-orange-600 flex flex-col  p-4">
							<h1>CATEGORIES</h1>
						...
						<div className="border border-gray-200 rounded-md  accent-orange-600 flex flex-col  p-4 my-2">
							<h1>SUB CATEGORIES</h1>
							{subCategoriesOptions.map((items, index) => {
								return (
									<div className="flex items-center gap-2 px-2" key={index}>
										<input
											type="checkbox"
											checked={items.value === subCategory}
											name="All" id="All"
											className="h-4 w-4"
											onClick={() => handleCategoryBy(items.value)}
										/>
						...
						<div className="border border-gray-200 rounded-md  accent-orange-600 flex flex-col p-4">
							<h1>AVAILABILITY</h1>
							<div className="flex">
								<input type="checkbox" checked={availability === 'EMBOSE'}
									name="EMBOSE"
									id="EMBOSE"
									className="my-2 mx-2 h-4 w-4"
									onClick={() => handleAvailability('EMBOSE')} />EMBOSE
							</div>
							<div className="flex">
								<input type="checkbox" checked={availability === 'OUT-OF-STOCK'}
									name="OUT OF STOCK"
									id="OUT-OF-STOCK"
									className="my-2 mx-2 h-4 w-4"
									onClick={() => handleAvailability('OUT-OF-STOCK')} />OUT OF STOCK
							</div>
						</div>
					</div>
					<div className=" flex flex-wrap w-[80%]">
						{items.map((item, index) => (
							<div key={index} className="cursor-pointer py-2 m-1 w-1/5 border border-gray-200 rounded-md">
								<img src={item.img} alt="" className="h-45 w-full object-cover rounded-md" />
								<div className="p-3">
